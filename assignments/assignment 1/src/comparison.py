...
def compare_results():
    ...
    img_search_csv = f"../out/hist_image_0001.jpg.csv"
    img_search_df = pd.read_csv(img_search_csv)
    img_search_top5 = set(img_search_df["Filename"].iloc[1:6])  # skipping the first row (which is the chosen image itself)

    # loading the results from vgg16 model
    vgg16_csv = f"../out/vgg16_image_0001.jpg.csv"
    vgg16_df = pd.read_csv(vgg16_csv)
    vgg16_top5 = set(vgg16_df["Filename"].iloc[1:6])  # skipping the first row (which is the chosen image itself)
